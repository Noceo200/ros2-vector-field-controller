...
kr_dist = 0.5 #coeff de repoussement (exponential position), can be negative
kr_force = 1.0 #1.0 #mutiply repulsive force
...
def vector_field(x,y,q_points, p_point):  #-grad(V) #x1 and x2 can be matrix because of draw_vector_field function
    ...
    """
    cl_rep_cmd_x, cl_rep_cmd_y = compute_rep_cmd(x,y,q_points,kr_dist,max_spd_norm)
    ...
    #this is always a non-matrix float, it is treated as a fixed speed command to apply on all the field.
...
def compute_rep_cmd(x,y,q_points,kr_dist,max_spd_norm): #x and y should be float, not matrix or list
    minus_grad_v_x = 0.0
    minus_grad_v_y = 0.0
...
def compute_attr_cmd(x,y,p_point,cmd_vel,ka_force,min_spd_norm,max_spd_norm,null_spd_radius):
    point_mode = False
    if norm(cmd_vel) > null_spd_radius: #then we should apply the received CMD rather than doing the point following
        ...
        if norm(cmd_d_vect) > 0.0: #other remap ratio keep default value of 0.0
            remap_ratio = clamp_not_zero(norm(cmd_d_vect),min_spd_norm,max_spd_norm,0.0)/norm(cmd_d_vect)

        cl_attr_cmd_x = cmd_d_x*remap_ratio
        cl_attr_cmd_y = cmd_d_y*remap_ratio
...
for t in arange(0,50,dt):
    ...
    #teleoperation cmd
    #cmd_vel = array([[0.5*cos((t*2)/2)], [0.5*sin((t*2)/2)]])
